/*
 * This code is adapted from the ProtocolLib project:
 * https://github.com/dmulloy2/ProtocolLib/blob/804aa5df11de975337b6947dd59528b432756d20/src/main/java/com/comphenix/protocol/injector/netty/channel/ChannelProtocolUtil.java
 * Copyright (C) 2025 ProtocolLib Project (original authors and contributors)
...
final class ChannelProtocolUtil {
	...
	private static final class Post1_20_5WrappedResolver implements BiFunction<Channel, PacketType.Sender, Object> {
		...
		public Object apply(Channel channel, PacketType.Sender sender) {
			...
			Object handler = context.handler();
			Function<Object, Object> protocolAccessor = this.getProtocolAccessor(handler.getClass(), sender);
			return protocolAccessor.apply(handler);
		}
		...
		private String getHandlerName(PacketType.Sender sender) {
			switch (sender) {
				case SERVER:
					return "encoder";
				case CLIENT:
