...
public class ObfuscationProcessor {
  ...
  public void process(ObfuscationRequest request) {
    var timer = statistics.injector.packetDelayProcessor.start();
    try {
  ...
  private void processInternal(ObfuscationRequest request) throws Exception {
    ChunkPacketAccessor packet = request.packetAccessor();
    WorldAccessor worldAccessor = packet.world();
    ...
    try (Chunk chunk = this.chunkFactory.fromPacket(packet)) {
      for (int sectionIndex = Math.max(0, bundle.minSectionIndex()); sectionIndex <= Math
        ...
        for (int index = 0; index < 4096; index++) {
          ...
          if (isObfuscateBitSet && obfuscationConfig.shouldObfuscate(y) && shouldObfuscate(request, chunk, x, y, z)) {
      ...
      request.complete(chunk.finalizeOutput(), blockEntities, proximityBlocks);
    }
  }
  ...
  private boolean shouldObfuscate(ObfuscationRequest request, Chunk chunk, int x, int y, int z) {
    return isAdjacentBlockOccluding(request, chunk, x, y + 1, z)
        && isAdjacentBlockOccluding(request, chunk, x, y - 1, z)
        && isAdjacentBlockOccluding(request, chunk, x + 1, y, z)
        && isAdjacentBlockOccluding(request, chunk, x - 1, y, z)
        && isAdjacentBlockOccluding(request, chunk, x, y, z + 1)
        && isAdjacentBlockOccluding(request, chunk, x, y, z - 1);
  }

  private boolean isAdjacentBlockOccluding(ObfuscationRequest request, Chunk chunk, int x, int y, int z) {
    ...
    return blockId >= 0 && registryAccessor.isOccluding(blockId);
  }
}
